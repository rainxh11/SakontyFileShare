...
namespace DeviceId
{
    ...
    public static class DeviceIdBuilderExtensions
    {
        ...
        public static DeviceIdBuilder AddMotherboardInfo(this DeviceIdBuilder builder, string info)
        {
            if (OS.IsWindows)
            {
                return builder.AddComponent(new WmiDeviceIdComponent($"Motherboard{info}", "Win32_BaseBoard", info));
            }
            else
            {
                return builder.AddComponent(new UnsupportedDeviceIdComponent("MotherboardInfo"));
            }
        }

        public static DeviceIdBuilder AddProcessorName(this DeviceIdBuilder builder)
        {
            if (OS.IsWindows)
            {
                return builder.AddComponent(new RegistryValueDeviceIdComponent("ProcessorNameString", @"HKEY_LOCAL_MACHINE\HARDWARE\DESCRIPTION\System\CentralProcessor\0", "ProcessorNameString"));
            }
            else
            {
                return builder.AddComponent(new UnsupportedDeviceIdComponent("MotherboardInfo"));
